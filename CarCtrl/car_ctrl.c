...
void car_wheel_speed_set(uint16_t s1, uint16_t s2, uint16_t s3, uint16_t s4)
{
    ...
    PWMPulseWidthSet(PWM0_BASE, PWM_OUT_0, s4*(PWMGenPeriodGet(PWM0_BASE, PWM_GEN_0)/100));//4
    PWMPulseWidthSet(PWM0_BASE, PWM_OUT_1, s3*(PWMGenPeriodGet(PWM0_BASE, PWM_GEN_0)/100));//3
    PWMPulseWidthSet(PWM0_BASE, PWM_OUT_2, s1*(PWMGenPeriodGet(PWM0_BASE, PWM_GEN_1)/100));//1
    PWMPulseWidthSet(PWM0_BASE, PWM_OUT_3, s2*(PWMGenPeriodGet(PWM0_BASE, PWM_GEN_1)/100));//2
}

void car_wheel_dir_set(bool w1, bool w2, bool w3, bool w4)
{
    GPIOPinWrite(GPIO_PORTA_BASE, GPIO_PIN_5, ((w4==1)?0x00:0xFF));//4. PA5, PA6
    GPIOPinWrite(GPIO_PORTA_BASE, GPIO_PIN_6, ((w4==0)?0x00:0xFF));

    GPIOPinWrite(GPIO_PORTA_BASE, GPIO_PIN_7, ((w3==1)?0x00:0xFF));//2. PA7, PD2
    GPIOPinWrite(GPIO_PORTD_BASE, GPIO_PIN_2, ((w3==0)?0x00:0xFF));
    
    GPIOPinWrite(GPIO_PORTD_BASE, GPIO_PIN_3, ((w1==1)?0xFF:0x00));//3. PD3, PE2  ****
    GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_2, ((w1==0)?0xFF:0x00));
    
    GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_3, ((w2==1)?0xFF:0x00));//1. PE3, PC5   ****
    GPIOPinWrite(GPIO_PORTC_BASE, GPIO_PIN_5, ((w2==0)?0xFF:0x00));
}
